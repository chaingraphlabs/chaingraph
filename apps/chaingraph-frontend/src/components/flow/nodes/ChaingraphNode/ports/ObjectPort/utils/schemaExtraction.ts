...
import { isAnyPortConfig } from '@badaitech/chaingraph-types'
import {
  isArrayPortConfig,
  isBooleanPortConfig,
  isEnumPortConfig,
...
export function extractNodeSchema(node: INode): ExtractedSchema {
  console.log('Extracting schema from node:', node.id, node.metadata.title)
  ...
  for (const port of inputPorts) {
    ...
    // Skip system ports (like __execute, __error, etc.)
    if (config.metadata?.isSystemPort) {
      continue
    }

    // Extract schema property from port
    const schemaProperty = portConfigToJsonSchema(port)
  ...
  for (const port of outputPorts) {
    ...
    // Skip system ports (like __success, __error, etc.)
    if (config.metadata?.isSystemPort) {
      continue
    }
    ...
    // Extract schema property from port
    const schemaProperty = portConfigToJsonSchema(port)
    outputProperties[key] = schemaProperty
