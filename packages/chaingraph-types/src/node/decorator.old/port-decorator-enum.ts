...
import type { EnumPortConfig, IPortConfig, PortType } from '@chaingraph/types/port-new/base'
...
export function PortEnumOf<T>(
  options: Array<T>,
  config?: PartialPortConfig<'enum'>,
) {
  return function (target: any, propertyKey: string) {
    const optionConfigs: IPortConfig[] = options.map((option, index) => {
    ...
    const enumConfig: EnumPortConfig = {
      ...config,
      type: 'enum',
      ...
      }, // Must be the id of the selected option
    }

    return Port<'enum'>(enumConfig)(target, propertyKey)
  }
}
