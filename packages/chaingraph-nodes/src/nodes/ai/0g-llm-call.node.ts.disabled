...
class OGLLMCallNode extends BaseNode {
  ...
  @Secret({
    ...
    description: 'Your 0G network private key used for authentication and transactions. This key is required to access the 0G network services and will be securely encrypted.',
    secretType: '0g',
  })
  privateKey?: EncryptedSecretValue<'0g'>
  ...
  async execute(ctx: ExecutionContext): Promise<NodeExecutionResult> {
    ...
    const privateKey = await this.privateKey.decrypt(ctx)
