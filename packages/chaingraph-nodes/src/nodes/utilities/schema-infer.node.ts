...
class SchemaInferNode extends BaseNode {
  ...
  updateDerivedSchema() {
    ...
    // Find output port
    const outputPort = this.getPort('output') as AnyPort | undefined
    if (!outputPort) {
      return
    }
  ...
  async execute(context: ExecutionContext): Promise<NodeExecutionResult> {
    // Schema inference already happened in @OnPortUpdate (design time)
    // At execution time, just parse and set the value
    const inputValue = this.input?.trim()
    if (!inputValue) {
      this.output = null
      return {}
    ...
    try {
      ...
      // Set the value on the already-configured output port
      this.output = parsed
    } catch (error) {
      // Parse error at runtime - set null
      this.output = null
    }
